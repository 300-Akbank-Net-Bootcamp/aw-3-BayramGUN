    ...
    IRequestHandler<GetContactByParameterQuery, ApiResponse<List<ContactResponse>>>
{
    ...
    public async Task<ApiResponse<List<ContactResponse>>> Handle(
        GetContactByParameterQuery request, 
        CancellationToken cancellationToken)
    {
         var list =  await dbContext.Set<Contact>()
            .Include(x => x.Customer)
            .ThenInclude(c => $"{c.FirstName} {c.LastName}")
            .Where(x => string.Equals(x.Information, request.Information, StringComparison.OrdinalIgnoreCase) ||
                string.Equals(x.Customer.FirstName, request.CustomerFirstName, StringComparison.OrdinalIgnoreCase) ||
                string.Equals(x.Customer.LastName, request.CustomerLastName, StringComparison.OrdinalIgnoreCase))
